...
export class Telegram {
  ...
  handleUpdate(update) {
    ...
    if (/^\/(start|help|status)(?:@\w+)?(?: |$)/.test(command)) {
      return `Bot Medicina conectado. Monitor de prints ${this.ledger.get('paused') === 'true' ? 'pausado no painel' : 'ativo'}. Salve um print novo na pasta Prints medicina para receber a letra e uma justificativa curta.\n\nVocê também pode escrever uma palavra, sigla ou dúvida de medicina aqui. Explicarei o conceito, as pistas de questões e as principais diferenças para ajudar a responder, mesmo sem o enunciado.`;
    }
    ...
    const job = this.sendChain.then(async () => {
      ...
      for (const part of parts) {
        ...
        for (let attempt = 0; ; attempt++) {
          ...
          try {
            result = await this.request('sendMessage', { chat_id: this.target, text: part, entities, reply_parameters, link_preview_options: { is_disabled: true }, reply_markup:{remove_keyboard:true} });
