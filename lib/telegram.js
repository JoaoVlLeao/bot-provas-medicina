...
export class Telegram {
  constructor({ token, ledger, expectedPhone = '', fetchImpl = fetch, wait = sleep, now = Date.now }) {
    Object.assign(this, { token, ledger, expectedPhone, fetchImpl, wait, now });
    ...
    const job = this.sendChain.then(async () => {
      ...
      for (const part of parts) {
        ...
        for (let attempt = 0; ; attempt++) {
          ...
          try {
            result = await this.request('sendMessage', { chat_id: this.target, text: part, entities, link_preview_options: { is_disabled: true }, reply_markup:{remove_keyboard:true} });
