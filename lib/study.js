...
export class StudyWorker {
  constructor({ledger,answer,send,isReady,canReply}) {
    Object.assign(this,{ledger,answer,send,isReady,canReply});this.task=null;this.error=null;
  ...
  enqueue({updateId,chatId,messageId,text}) {
    ...
    if(!this.canReply(chatId) || !Number.isSafeInteger(updateId) || !Number.isSafeInteger(messageId) || messageId<=0 || !topic || topic.length>4096) return;
  ...
  status() {
    return {error:this.error,counts:Object.fromEntries(this.ledger.db.prepare('SELECT status,COUNT(*) n FROM study_messages GROUP BY status').all().map(r=>[r.status,r.n])),recent:this.ledger.db.prepare('SELECT update_id,chat_id,substr(topic,1,100) name,status,error,updated FROM study_messages ORDER BY updated DESC LIMIT 10').all()};
  }
  tick() {
  ...
  async drain() {
    while(this.isReady()) {
      const item=this.ledger.db.prepare("SELECT * FROM study_messages WHERE status='queued' AND retry_at<=? ORDER BY update_id LIMIT 1").get(Date.now());
      if(!item) break;
      if(!this.canReply(item.chat_id)) {this.update(item.update_id,{status:'failed',error:'Conversa de estudo não autorizada.'});continue;}
