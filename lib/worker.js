...
export class Worker {
  ...
  async process(file) {
    ...
    try {
      ...
      this.ledger.update(file.id,{status:'sending',target:this.target});sending=true;
      const result=await this.send(this.target,answer);
...
export function safeError(e) {return String(e.message||'Erro desconhecido').replace(/AIza[\w-]+/g,'[chave omitida]').replace(/Bearer\s+\S+/gi,'Bearer [omitido]').replace(/https?:\/\/\S+/g,'[endereço omitido]').slice(0,250);}
