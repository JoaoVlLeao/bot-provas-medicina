...
<div id="telegram-pair" hidden><p>Abra o link abaixo com a conta que receberá as respostas, toque em <strong>Iniciar</strong> e confirme seu número quando solicitado.</p><p id="expected-number"></p>
...
<p class="note">Estudo por mensagem restrito aos números autorizados, com verificação do próprio contato pelo Telegram. Envie uma palavra, sigla ou dúvida de medicina e receba a explicação na própria conversa. Os prints do Drive continuam exclusivos do destinatário configurado.</p>
...
<section class="card activity"><h2>Estudo por mensagem</h2><p id="study-summary">Envie um tema de medicina no Telegram.</p><div id="study-activity"></div></section>
...
async function refresh(){try{
 ...
 el('expected-number').textContent=t.expectedPhone?'Destino autorizado: +'+t.expectedPhone:'';
 ...
 const study=s.study||{counts:{},recent:[]};el('study-summary').textContent=(study.counts.sent||0)+' explicações enviadas · '+((study.counts.queued||0)+(study.counts.analyzing||0)+(study.counts.sending||0))+' em andamento · '+((study.counts.failed||0)+(study.counts.uncertain||0))+' a conferir';
 el('study-activity').replaceChildren();for(const item of study.recent){const row=document.createElement('div');row.className='row';const topic=document.createElement('span');topic.textContent=item.name;const state=document.createElement('span');state.textContent=names[item.status]||item.status;row.append(topic,state);if(item.error){const error=document.createElement('small');error.textContent=item.error;row.append(error);}el('study-activity').append(row);}
 if(!el('study-activity').children.length)el('study-activity').textContent='Exemplos: choque neurogênico, síndrome nefrótica ou diferença entre asma e DPOC.';
